...
class ShopPosPieMethodView extends StatelessWidget {
  final controller = Get.put(PosShopPieMethodController());
  ...
  Widget build(BuildContext context) {
    return Container(
      child: Column(
        children: [
          GetBuilder<PosShopPieMethodController>(builder: (controller) {
  ...
  timeLineWidget(PosShopPieMethodController controller) {
    return Container(
      padding:
      ...
      child: Column(
        children: [
          ...
          controller.amountPerShop == LocaleKeys.select_shop_to_view_pos_method.tr
  ...
  amountTextWidget(PosShopPieMethodController controller, {title}) {
    return BiaText(
      title ?? "",
  ...
  showTimeLineWidget(PosShopPieMethodController controller, bool isShop) {
    return Container(
      height: 40.h,
              ...
              onTap: () {
                ...
                if (controller.posTimeModelList.value[index].id == 1) {
                  controller.hitApiToGetShopPerPaymentCategory(
                      timeUrl: MERCHENT_COLLECT_TODAY_PAYMENT_METHOD_POS +
                          '${controller.shopId?.toString()}');
                } else if (controller.posTimeModelList.value[index].id == 2) {
                  controller.hitApiToGetShopPerPaymentCategory(
                      timeUrl:
                          MERCHENT_COLLECT_THIS_WEEK__PAYMENT_METHOD_POS +
                              '${controller.shopId?.toString()}');
                } else if (controller.posTimeModelList.value[index].id == 3) {
                  controller.hitApiToGetShopPerPaymentCategory(
                      timeUrl:
                          MERCHENT_COLLECT_SEVEN_DAYS__PAYMENT_METHOD_POS +
                              '${controller.shopId?.toString()}');
                } else if (controller.posTimeModelList.value[index].id == 4) {
                  controller.hitApiToGetShopPerPaymentCategory(
                      timeUrl:
                          MERCHENT_COLLECT_THIS_MONTH__PAYMENT_METHOD_POS +
                              '${controller.shopId?.toString()}');
                } else if (controller.posTimeModelList.value[index].id == 5) {
                  controller.hitApiToGetShopPerPaymentCategory(
                      timeUrl:
                          MERCHENT_COLLECT_LAST_MONTH__PAYMENT_METHOD_POS +
                              '${controller.shopId?.toString()}');
                }
              },
              child: Column(
  ...
  pieChartByPaymentMethodWidget(PosShopPieMethodController controller) {
    return PieChart(
  ...
  posColletedAmountWidget(
      PosShopPieMethodController controller, bool isMethod) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          headingTextWidget(
              title: LocaleKeys.total_amount_collected_pos_pie_method.tr),
          SizedBox(
            height: 20.h,
          ),
  ...
  headerWidget(PosShopPieMethodController controller, isShop) {
    return Row(
      children: [
  ...
  shopSelectionDropWidget(PosShopPieMethodController controller, bool isShop) {
  ...
  currencySelectionDropWidget(
      PosShopPieMethodController controller, bool isShop) {
    return CustomSelectView(
      onTab: () {
  ...
  showShopSelectionDialog(PosShopPieMethodController controller, bool isShop) {
  ...
  posSelectionDropWidget(PosShopPieMethodController controller) {
    return CustomSelectView(
      onTab: () {
  ...
  showPosSelectionDialog(PosShopPieMethodController controller) {
    return showGenerateDialog(
        context: Get.overlayContext!,
        child: Column(
          mainAxisSize: MainAxisSize.min,
